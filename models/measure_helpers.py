...
def measure_model(
    model_version, dataset, out_folder, weights_dir, device, sample_images=25
):
    ...
    print(f"Measuring {model_version} on {dataset} dataset")
    print("-" * 10)
    pbar = tqdm(total=test_dataset.__len__(), desc="Model test completion")
    integrated_gradients = GuidedGradCam(model, model.conv1)
    ...
    for input, label in data_loader:
        ...
        attributions_ig = nt.attribute(
            input, nt_type="smoothgrad", nt_samples=6, target=pred_label_idx
        )

        infid = infidelity(
            model, perturb_fn, input, attributions_ig, target=pred_label_idx
        )
        sens = sensitivity_max(nt.attribute, input, target=pred_label_idx)
    ...
    np.savetxt(
        os.path.join(out_folder, f"{model_version}-{dataset}.csv"),
        np.array(scores),
        delimiter=",",
